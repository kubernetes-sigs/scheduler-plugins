...
func TestNodeResourceFitplus(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			nodeInfos: []*framework.NodeInfo{makeNodeInfo("machine1", "96", "512Gi", "2"), makeNodeInfo("machine2", "96", "512Gi", "6")},
			args: config.NodeResourcesFitPlusArgs{
				Resources: map[v1.ResourceName]config.ResourcesType{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			snapshot := newTestSharedLister(pods, test.nodeInfos)
...
func makeNodeInfo(node string, cpu, memory, gpu string) *framework.NodeInfo {
