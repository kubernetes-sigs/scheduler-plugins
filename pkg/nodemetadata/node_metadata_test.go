...
func TestValidateArgs(t *testing.T) {
	tests := []struct {
		...
		{
			name: "timestamp without format uses default validation path",
			args: &config.NodeMetadataArgs{
				MetadataKey:     "timestamp",
				MetadataSource:  config.MetadataSourceAnnotation,
				MetadataType:    config.MetadataTypeTimestamp,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validation.ValidateNodeMetadataArgs(tt.args, nil)
			if (err != nil) != tt.expectError {
				t.Errorf("ValidateNodeMetadataArgs() error = %v, expectError %v", err, tt.expectError)
			}
		})
	}
}
...
func TestNewDefaultsTimestampFormat(t *testing.T) {
	args := &config.NodeMetadataArgs{
		MetadataKey:     "last-update",
		MetadataSource:  config.MetadataSourceAnnotation,
		MetadataType:    config.MetadataTypeTimestamp,
	...
	nm, ok := plugin.(*NodeMetadata)
	if !ok {
		t.Fatalf("New() returned %T, want *NodeMetadata", plugin)
	}

	if nm.args.TimestampFormat != time.RFC3339 {
		t.Fatalf("TimestampFormat = %q, want %q", nm.args.TimestampFormat, time.RFC3339)
	}
}
