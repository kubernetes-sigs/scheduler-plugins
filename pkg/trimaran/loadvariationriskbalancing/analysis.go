...
func (rs *resourceStats) computeScore(margin float64, sensitivity float64) float64 {
	...
	risk := (mu + sigma) / 2
	klog.V(6).InfoS("mu=", "mu", mu, " sigma=", "s", sigma, " margin=", "margin", margin, 
	   " sensitivity=", "sensitivity", sensitivity, " risk=", "risk", risk)
...
func createResourceStats(metrics []watcher.Metric, node *v1.Node, podRequest *framework.Resource,
	...
	if !metricFound {
		klog.V(6).InfoS("Rsource usage statistics for node : no valid data", " node", klog.KObj(node))
		return nil, false
	}
