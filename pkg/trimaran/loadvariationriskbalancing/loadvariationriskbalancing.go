...
func (pl *LoadVariationRiskBalancing) Score(ctx context.Context, cycleState *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
	...
	klog.V(6).InfoS("Calculating score for", "pod",  klog.KObj(pod), "node", nodeName)
	...
	klog.V(6).InfoS("Calculating CPUScore for", "pod", klog.KObj(pod), "nodeName", nodeName, "cpuScore", cpuScore)
