...
func TestClone(t *testing.T) {
	...
	cloned := original.Clone().(PodsInfo)

	storedPod, ok := cloned["pod-a"]
	if !ok {
	...
	delete(cloned, "pod-a")
	if _, ok := original["pod-a"]; !ok {
		t.Fatal("deleting from clone should not affect original")
	}
}

func TestPreFilter(t *testing.T) {
	tm := &TopologyMatch{preemptionMode: apiconfig.PreemptionEnabled}
	...
	if err != nil {
		t.Fatalf("failed to read PodsInfo after PreFilter: %v", err)
	}
	if ps == nil {
		t.Fatal("PodsInfo map should be initialized")
	}
	if len(ps) != 0 {
		t.Fatalf("PodsInfo should be empty, got %d entries", len(ps))
	}
}
...
func TestRemovePod(t *testing.T) {
	...
	if len(ps) != 3 {
		t.Fatalf("expected 3 victim pods, got %d", len(ps))
	}
	if _, ok := ps["ns/victim-1"]; !ok {
		t.Error("victim-1 missing")
	}
	if _, ok := ps["ns/victim-2"]; !ok {
		t.Error("victim-2 missing")
	}
	if _, ok := ps["ns/victim-3"]; !ok {
...
func TestAddPod_EmptyStack(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to read PodsInfo before AddPod: %v", err)
	}
	victim1 := makeTestPod("ns", "victim-1", "c1")
	status := tm.AddPod(context.Background(), cycleState, nil, makeTestPodInfo(victim1), nil)
	if !status.IsSuccess() {
	...
	if err != nil {
		t.Fatalf("failed to read PodsInfo after AddPod: %v", err)
	}
	if !maps.Equal(psBefore, psAfter) {
		t.Fatalf("expected same PodsInfo after AddPod to empty stack")
	}
}
...
func TestGetPods(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			sort.Slice(tc.expectedPods, func(i, j int) bool {
				return tc.expectedPods[i].Namespace+"/"+tc.expectedPods[i].Name < tc.expectedPods[j].Namespace+"/"+tc.expectedPods[j].Name
			})
...
func TestAddRemovePod_PreemptionDisabled(t *testing.T) {
	tm := &TopologyMatch{preemptionMode: apiconfig.PreemptionDisabled}
	cycleState := framework.NewCycleState()
	tm.PreFilter(context.Background(), cycleState, makeTestPod("ns", "scheduler"), nil)

	victim1 := makeTestPod("ns", "victim-1", "c1")

	status := tm.RemovePod(context.Background(), cycleState, nil, makeTestPodInfo(victim1), nil)
	if !status.IsSuccess() {
		t.Fatalf("RemovePod returned non-success: %v", status)
	}
	_, err := readPreemptionStack(cycleState)
	if err == nil {
		t.Fatalf("expected error reading PodsInfo after RemovePod, got nil")
	}
