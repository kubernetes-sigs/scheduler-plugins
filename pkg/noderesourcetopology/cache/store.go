...
func getNUMAPlacementInfo(lh logr.Logger, nrt topologyv1alpha2.NodeResourceTopology, pods []podData) numaplacement.EncodedInfo {
	...
	if len(cnts) != payload.Containers {
		lh.V(2).Error(err, "inconsistent containers count", "node", nname, "computed", payload.Containers, "expected", len(cnts))
		return numaplacement.EncodedInfo{}
	}
	if len(cnts) == 0 {
	...
	if !ok {
		lh.V(2).Error(err, "failed to cast node info to encoded info", "node", nname)
		return numaplacement.EncodedInfo{}
	}
