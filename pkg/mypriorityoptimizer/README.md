...
This project introduces an **optimized, priority-based** approach for placing pods onto nodes via the **MyPriorityOptimizer** plugin for the Kubernetes scheduler. The project is a fork of the Kubernetes-sigs project [scheduler-plugins](https://github.com/kubernetes-sigs/scheduler-plugins) and extends it with a new plugin (see describtion of the [Scheduling Framework](https://kubernetes.io/docs/concepts/scheduling-eviction/scheduling-framework/)).

The goal is to schedule as many *high-priority* pods as possible, especially when the *default scheduler* fails to do so (a side effect of this approach is also better cluster utilization). This is achieved by *preempting* (evicting and moving) already running *low-priority* pods, possibly across multiple nodes in the cluster, to make room for the high-priority pods.

The plugin enables integration of an *external* solver—here, a Python solver using **[Google's CP-SAT](https://developers.google.com/optimization/cp/cp_solver)**—to compute an optimal placement plan that maximizes scheduled high-priority pods while *minimizing disruption* by reducing the number of preemptions (*movements* and *evictions*). Given the solver's plan, the plugin applies it by evicting and moving pods, possibly across multiple nodes in the cluster. Note that the default Kubernetes scheduler can only preempt pods within a *single* node.
...
The plugin can be integrated in different **scheduling phases**: either before enqueuing the pod (*PreEnqueue*) or after the default scheduler has failed to place it (*PostFilter*). We recommend the last option, as it skips optimization when the default scheduler can already place the pod and leverages its speed instead of running unnecessary solver logic.
...
To enable and use plugins in the Kubernetes scheduler, you must apply a **scheduler configuration manifest** that selects the plugins and their settings; the manifest for this plugin is located in `bootstrap/content/manifests/` (see also [Scheduler Configuration on Kubernetes webpage](https://kubernetes.io/docs/reference/scheduling/config/)).

Here you will notice two files:

- `plugin-kube-scheduler-config.yaml`: Configuration for using the **MyPriorityOptimizer** plugin to configure the scheduler with optimization approach.
...
The reason that we also have a MyScoreBreaker plugin, located under `pkg/myscorebreaker/`, is that when generating test jobs, we want to be able to reproduce the same seeds where not all pods are running when using the default scheduler. Therefore, we have created a simple plugin that breaks scoring ties by name to make scheduling deterministic. More details later in [Testing](#testing).

These plugins are referenced and registered in `cmd/scheduler/main.go` such that the scheduler will include them at build time.
...
2. Build the latest scheduler binary or Docker image with the plugin enabled (see [Building](#building)).
...
4) Create a Terminal instance with Ubuntu 22.04. A illustration is shown below:
   ...
   ![UCloud Terminal Example](./ucloud_job_example.png)
