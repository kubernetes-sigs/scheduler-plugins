...
func (pl *MyCrossNodePreemption) setActivePlan(sp *StoredPlan, id string, pods []*v1.Pod) {
	...
	// Build from NewPlacements:
	// - WorkloadPerNodeCnts (controllerName -> node -> remaining count)
	// - PlacementByName for standalone pods and the preemptor (even if RS-owned)
	remaining := make(WorkloadPerNodeCnts)
	byName := make(map[string]string, len(sp.NewPlacements)+1)

	podMap := podsByUID(pods)
	preUID := ""
	if sp.Preemptor != nil {
		preUID = sp.Preemptor.Pod.UID
	}

	for _, plm := range sp.NewPlacements {
		p := podMap[plm.Pod.UID]
		// If we cannot resolve the pod reliably, skip conservative
		if p == nil || string(p.UID) != plm.Pod.UID {
			continue
		}

		// Preemptor is always placed using its name (also if a replicapod)
		if preUID != "" && plm.Pod.UID == preUID {
			byName[combineNsName(plm.Pod.Namespace, plm.Pod.Name)] = plm.ToNode
			continue
		}

		// Controller-owned → quota by controller name; Standalone pods → pin by name
		if wk, owned := topWorkload(p); owned {
			// Use the controller (e.g., ReplicaSet) name (no autogenerated pod name suffixes)
			wkKey := wk.String()
			if remaining[wkKey] == nil {
				remaining[wkKey] = make(map[string]*atomic.Int32)
			}
			if remaining[wkKey][plm.ToNode] == nil {
				remaining[wkKey][plm.ToNode] = new(atomic.Int32)
			}
			remaining[wkKey][plm.ToNode].Add(1)
		} else {
			// Standalone pod: place by exact name
			byName[combineNsName(plm.Pod.Namespace, plm.Pod.Name)] = plm.ToNode
		}
	}

	// Also pin preemptor to its nominated node if provided (source of truth)
