...
- 
- Instead of having workload cnt in storedPlan only use NewPlacement but only use the replicaset name (dont use their autogenerated name-ending), thus we can count how many that should fit on each node using only NewPlacement. Just keep their UIDs for reference and to be able to delete them when we need to evict them.
