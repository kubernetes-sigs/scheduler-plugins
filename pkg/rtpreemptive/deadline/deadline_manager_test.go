...
func (suite *DDLManagerSuite) SetupTest() {
	...
	suite.existingPod = st.MakePod().UID(existingPodUID).Annotations(map[string]string{AnnotationKeyDDL: "10s"}).CreationTimestamp(metav1.NewTime(time.Now())).Obj()
...
func (suite *DDLManagerSuite) TestParsePodDeadline() {
	...
	now := time.Now()
	pod := st.MakePod().Annotations(map[string]string{AnnotationKeyDDL: "10s"}).CreationTimestamp(metav1.NewTime(now)).Obj()
...
func (suite *DDLManagerSuite) TestAddPodDeadline() {
	manager := deadlineManager{podDeadlines: suite.deadlineCache}
	now := time.Now()
	pod := st.MakePod().Annotations(map[string]string{AnnotationKeyDDL: "20s"}).CreationTimestamp(metav1.NewTime(now)).Obj()
...
func (suite *DDLManagerSuite) TestGetPodDeadline() {
	manager := deadlineManager{podDeadlines: suite.deadlineCache}
	now := time.Now()
	pod := st.MakePod().Annotations(map[string]string{AnnotationKeyDDL: "30s"}).CreationTimestamp(metav1.NewTime(now)).Obj()
