...
type NodeResourceTopologyLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.NodeResourceTopology, err error)
	// Get retrieves the NodeResourceTopology from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.NodeResourceTopology, error)
...
// List lists all NodeResourceTopologies in the indexer.
func (s *nodeResourceTopologyLister) List(selector labels.Selector) (ret []*v1alpha2.NodeResourceTopology, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.NodeResourceTopology))
	})
	return ret, err
}

// Get retrieves the NodeResourceTopology from the index for a given name.
func (s *nodeResourceTopologyLister) Get(name string) (*v1alpha2.NodeResourceTopology, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("noderesourcetopology"), name)
	}
	return obj.(*v1alpha2.NodeResourceTopology), nil
}
